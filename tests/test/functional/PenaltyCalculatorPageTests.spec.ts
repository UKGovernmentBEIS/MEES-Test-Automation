...
test.describe('Penalty Calculator Page Navigation Tests', () => {
    let penaltyCalculatorPage: PenaltyCalculatorPage;
...
test.describe('Penalty Calculator Results Page Navigation Tests', () => {
    ...
    test('Should navigate to Home page when clicking Home breadcrumb link', async () => {
        const resultsPage = await penaltyCalculatorPage.calculateMaximumPenalty('Less than 3 months', 50000);
        const homePage = await resultsPage.clickBreadcrumbHome();
        expect(await homePage.isDisplayed()).toBe(true);
    });

    test('Should navigate to Penalty Calculator page when clicking Penalty Calculator breadcrumb link', async () => {
        const resultsPage = await penaltyCalculatorPage.calculateMaximumPenalty('Less than 3 months', 50000);
